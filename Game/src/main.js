...
locations.onLevelDone = (id, proceed) => {
  ...
  levelResult.onContinue = () => {
    ...
    // Площадке тут говорить нечего: за «Продолжить» идёт не игра, а заставка
    // следующего уровня. Здесь стоял `play()`, и следом `proceed` уводил в
    // загрузку, где сразу же встаёт `pause()`, — площадка получала отрезок
    // геймплея нулевой длины, которого не было. Геймплей начнётся своим
    // чередом, когда уровень откроется.
