...
export class PrefabLibrary {
  ...
  _describe(name) {
     ...
     *
     * Подвижное сюда не идёт вовсе — его просто расталкивают ногами. Оговорка
     * не лишняя: у мебели пороги пересекаются, и маленький стул подходил разом
     * под оба — выходил предмет, который и пинают, и перепрыгивают. Сама игра
     * прыжок ему потом не давала, но описание вещи врало, а по нему судит
     * всякий, кто его прочтёт.
     */
    const vault = !dynamic && (rules.vault === true
      || (Boolean(rules.vaultUnder) && Boolean(size) && size.y <= rules.vaultUnder));

    /**
     * За чем герой пропадает из виду.
     *
     * Правило одно на всю игру и считается по объёму вещи. Всё, что крупнее
     * порога, — машина, дом, контейнер, шкаф — прячет целиком: зашёл за такое,
     * и тебя не видно, хоть ты и присел за низким бортом. Всё, что мельче,
     * заслоняет только той частью, что выше пояса: за бочкой и ящиком не
     * спрячешься, а вот они сами закрывают стоящего за ними по грудь.
     *
     * По объёму, а не по высоте: диван низкий, но длинный и глубокий, и залечь
     * за ним можно, а торшер той же высоты не прячет никого. Поимённо такое
     * пришлось бы перечислять, и всякая новая модель из Blender приезжала бы без
     * правила.
     *
     * Явное `hides` в настройках сильнее порога — им отмечают то, что по
     * габаритам на укрытие не тянет, а по смыслу прячет.
     */
    const bulk = size ? size.x * size.y * size.z : 0;
    const hides = rules.hides === true
      || (Boolean(rules.hidesOver) && bulk >= rules.hidesOver);

    const sightShapes = hides
