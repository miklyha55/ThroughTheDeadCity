...
export class LocationManager {
  ...
  async load(id) {
    ...
    /**
     * Сперва картинка, потом показ — и ничего не ждём.
     *
     * Ждать больше нечего: заставки всех уровней разобраны ещё на общей
     * загрузке, и знакомому уровню `prepare` ставит картинку тем же тактом,
     * не отдавая управление. Поэтому заставка открывается уже с нужной.
     *
     * Порядок тут менялся дважды, и оба прежних были хуже. С ожиданием
     * распаковки перед показом экран на это время не закрыт ничем — сквозь него
     * видна живая игра. С показом раньше подготовки заставка открывается с
     * картинкой пройденного уровня, и та сменяется следующим тактом. Ни того,
     * ни другого теперь нет.
     *
     * Незнакомому уровню — открытому через адрес — картинку всё же приходится
     * дочитывать из файла, и он единственный, кому достанется прежняя на
     * мгновение. Ради него задерживать переход не стоит.
     */
    this.splash?.prepare(id);
    this.splash?.show();

    /**
     * Отдать браузеру кадр — и только потом строить уровень.
     *
     * Сборка локации занимает около двух десятых секунды сплошной работы, и
     * поток в это время не рисует ничего. Без этой уступки заставка, поднятая
     * строкой выше, на экран за эти две десятых так и не попадает: игрок
     * нажимает «Продолжить» и видит, как замирает экран итога. Заметно это
     * именно как рывок — кнопка нажата, а картинка прежняя.
     *
     * Два кадра, а не один: первый заканчивает раскладку поднятой заставки,
     * второй её рисует. После него замирать уже не на чем — сборка идёт под
     * неподвижной картинкой, и её никто не видит.
     */
