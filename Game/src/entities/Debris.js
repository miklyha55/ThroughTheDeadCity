...
export class Debris {
  ...
  add(object, body, floorY, { explosive = false, blade = false } = {}) {
    ...
    this.items.push({
      ...
      // Последнее место, где предмет стоял снаружи всех преград. Сюда его
      // возвращают, если он всё-таки оказался внутри одной из них. Пока такого
      // места не было — предмет так и расставили, внутри стены, — держать его
      // не за что, и `null` это значит.
      safeX: null,
      safeZ: null,
    });
  }
  ...
  _kick(item, moverPosition, moverRadius, moverSpeed, mover = null) {
    ...
    /**
     * Вынести предмет из тела того, кто его пнул, — но не сквозь стену.
     *
     * Сдвиг тут мгновенный и ничем не ограничен: иначе предмет остаётся стоять
     * в ногах и пинается заново каждый кадр. Пока вокруг пусто, это безобидно,
     * а вот в углу — нет. Там отходить некуда, и предмет, зажатый между двумя
     * преградами, этим сдвигом вдавливался прямо в забор, глубже, чем
     * выталкивание успевает вернуть за кадр. Со стороны это выглядело так,
     * будто ящик протаскивают за ограду.
     *
     * Поэтому сдвиг отменяется, если он загоняет предмет внутрь преграды.
     * Толчок при этом остаётся — предмет просто уезжает вдоль стены, а не
     * сквозь неё.
     */
    const nextX = moverPosition.x + dx * touch;
    const nextZ = moverPosition.z + dz * touch;
    const bottom = position.y + item.boxMin.y;
    if (this.location?.obstacles?.inside(nextX, nextZ, bottom)) return;
  ...
  _collideWalls(item) {
    ...
    /**
     * Последняя проверка: предмет не должен остаться внутри преграды.
     *
     * Выталкивание за кадр ограничено, и один проход по контурам в углу всего
     * не разбирает: две преграды толкают в разные стороны, часть хода
     * срезается пределом, и предмет остаётся сидеть в стене. Кадром позже его
     * вытолкнет уже через дальнюю грань — наружу, за забор.
     *
     * Возвращаем туда, где он последний раз стоял снаружи, и гасим ход: ему
     * некуда ехать, раз впереди стена. Пока такого места не случилось — предмет
     * расставили внутри преграды ещё в редакторе, — не держим его вовсе, иначе
     * он не сдвинется никогда.
     */
    const obstacles = this.location.obstacles;

    if (obstacles.inside(position.x, position.z, bottom)) {
      if (item.safeX === null) return; // вернуть некуда: его тут и поставили

      position.x = item.safeX;
      position.z = item.safeZ;
      item.velocity.x = 0;
      item.velocity.z = 0;
      return;
    }
    ...
    /**
     * Годным для возврата считается только место, где чисто по горизонтали
     * целиком, без оглядки на высоту. Точка над забором формально свободна —
     * предмет над ним пролетает, — но возвращать в неё нельзя: упав, он
     * окажется ровно в заборе, из которого мы его и вынимаем.
     */
    if (!obstacles.inside(position.x, position.z)) {
      item.safeX = position.x;
      item.safeZ = position.z;
    }
