...
export class Zombie extends Figure {
  ...
  update(dt, player, location, crowd) {
    ...
    /**
     * Скелет считаем только тому, кого видно или слышно.
     *
     * Отсечение по кадру гасит зомби вне экрана, но это про отрисовку: считать
     * их позы игра продолжала всё равно, всем до единого. На шоссе это две с
     * лишним сотни скелетов за кадр при десятке в кадре — остальное уходило в
     * никуда.
     *
     * Замерший вне экрана ничего не портит: клип зациклен, и вернувшись в кадр
     * зомби продолжит с того места, где встал, — ровно так же, как если бы он
     * всё это время шагал. Рывка нет, потому что нет и точки, к которой надо
     * возвращаться.
     *
     * Круг шире слышимости нарочно. Шаги отмеряются по времени клипа, и
     * замерший зомби беззвучен; за спиной у героя он как раз невидим — и вот
     * его-то слышать надо. `stepRange` — четырнадцать метров, `voiceRange` —
     * двадцать, круг взят с запасом от обоих.
     *
     * Видимость тут от прошлого кадра: отсечение идёт после хода мира, а не до
     * него. Это не недосмотр — на входе в кадр зомби оживает кадром позже, чего
     * не увидеть, а всё, что рядом, и так держится кругом.
     *
     * Шаги считаются после состояний, но ДО `mixer.update`: клип в этот миг
     * стоит там же, где его видит игрок в нынешнем кадре.
     */
    if (this.root.visible || distance <= CFG.animateRadius) {
      this._steps();
      this.mixer.update(dt);
    }
  }
  ...
  _separate(step, crowd, dt) {
    ...
    // Своё тело и общий множитель — один раз на всю толпу, а не на каждого
    // соседа: этот цикл проходит по всем, и в нём дорога каждая мелочь.
    const mine = this.bodyRadius;
    const spread = CFG.separation / (2 * CFG.bodyRadius);
    ...
    for (const other of crowd) {
      ...
      const apart = spread * (mine + other.bodyRadius);

      /**
       * Сперва по осям, потом по квадрату, и только для настоящих соседей —
       * корень.
       *
       * Цикл квадратичный: на шоссе в толпе две с лишним сотни зомби, то есть
       * за кадр он проходит больше пятидесяти тысяч пар. Расходятся из них
       * единицы — те, кто и правда стоит вплотную. Раньше на каждую пару
       * считался `Math.hypot`, и весь этот корень уходил в никуда: он один из
       * самых дорогих способов померить расстояние, потому что честно бережётся
       * от переполнения.
       *
       * Теперь дальние отсеиваются одним сравнением по оси, и до арифметики
       * доходят только те, кто рядом. Поведение прежнее до последнего знака:
       * порог тот же, просто проверенный дешевле.
       */
      const dx = position.x - other.position.x;
      if (dx > apart || dx < -apart) continue;

      const dz = position.z - other.position.z;
      if (dz > apart || dz < -apart) continue;

      const room = dx * dx + dz * dz;
      if (room >= apart * apart || room < 1e-8) continue;

      const gap = Math.sqrt(room);
