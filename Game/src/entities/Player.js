...
export class Player extends Figure {
  ...
  arm() {
    ...
    // Мгновение тишины после подбора — та же пауза, что и в начале уровня.
    // Без неё ружьё стреляло само в тот же кадр, в котором попало в руки: цель
    // выбирается сама, а зомби на вводной стоят в двух шагах от него. Игрок
    // только поднял оружие, а персонаж уже палит и лезет перезаряжаться.
    this.reloading = CFG.startDelay;
  }
