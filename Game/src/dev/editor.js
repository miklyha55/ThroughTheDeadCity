 ...
 *
 * Двигается всё, что стоит на локации, — и поштучные пропы, и россыпь. При
 * сохранении россыпь запекается: её предметы уходят в файл поимённо, а зоны из
 * него удаляются. Иначе правку было бы не сохранить: зона задаёт не места, а
 * правило, по которому они разыгрываются заново при каждой загрузке.
 *
 * Забор, орды зомби и выход мышью не трогаются — они правятся в JSON руками.
 *
  ...
  const status = (text) => {
    panel.textContent = active
      ? `правка: ${locations.current.data.name}\n`
        ...
        + 'сохранение запекает россыпь: зоны уйдут, предметы станут поимённо\n'
        + text
      : '';
  };
