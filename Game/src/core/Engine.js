...
export class Engine {
  constructor(container) {
    ...
    /**
     * И отдельно — за «видимой» частью страницы.
     *
     * На телефоне она живёт своей жизнью: адресная строка уезжает, всплывает
     * клавиатура, страницу растягивают щипком. Обычный `resize` на это молчит,
     * и холст остаётся прежнего размера — видно от него кусок, а остальное
     * показывает то, что было нарисовано раньше. Ровно так экран и разъезжался
     * на две несходящиеся половины.
     */
    window.visualViewport?.addEventListener('resize', this._resize);

    // Поворот телефона: размеры меняются не сразу, а через кадр после события,
    // и снятые в тот же миг оказываются старыми.
