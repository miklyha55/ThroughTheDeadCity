...
export class StartMessage {
   ...
   *
   * От пропуска отличается тем, что речь не объявляется договорённой: подсказка
   * по управлению привязана к её концу, а конца здесь не было — игрок просто
   * ушёл. От `reset` — тем, что вступление остаётся отзвучавшим: игрок слышал
   * начало и сам решил уйти, и встречать его той же речью при возвращении
   * незачем.
   */
  stop() {
    if (!this.locked) return;
