...
export class Splash {
  ...
  /**
   * Заставка на время загрузки самой игры — моделей, звука, библиотек.
   *
   * Название и картинка лежат в файле уровня, а его к этому моменту ещё не
   * читали: загрузка идёт задолго до того, как до локаций дойдёт черёд. Поэтому
   * сперва ставим запасную картинку — на экране сразу должно быть хоть что-то,
   * пустое место здесь означает чёрный экран, — а название и точную картинку
   * дописываем, как только файл прочитан.
   *
  ...
  boot(id) {
    ...
    // без await: заставка уже на экране, подпись догонит
    fetch(asset(`locations/${id}.json`))
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => data && this.setLevel(data.name, data.number ?? CFG.bootLevel))
      .catch(() => {}); // не прочиталось — заставка просто останется без подписи
  }
