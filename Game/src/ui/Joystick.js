...
export class Joystick {
  constructor(container = document.body) {
    ...
    /**
     * Долгое нажатие не должно звать выделение и «Найти в Яндексе».
     *
     * Отмены у `pointerdown` для этого мало, и это главное, что здесь стоит
     * помнить: указательные события — надстройка, а WebKit решает про выделение
     * и всплывающую панель по своим, касательным. До них отмена не доходит, и
     * на iOS игрок, задержавший палец на стике, получал поверх игры чужую
     * плашку с поиском и переводом. Гасим у самого источника.
     *
     * Непременно `passive: false`: без него браузер вправе считать слушатель
     * необязательным и отмену молча выбросить.
     */
    this.zone.addEventListener('touchstart', (e) => e.preventDefault(), { passive: false });

    // И меню по долгому нажатию — там, где оно вызывается правой кнопкой.
